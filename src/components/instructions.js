...
const Instructions = ({ language, inout, io, setInout, imageUrls, activeStep, setActiveStep, setStep, pictureIndex, setPictureIndex, pictureCount, setPictureCount, pictureSwipeDirection, intervalID, setIntervalID, slideShow, directions, setDirs, dirs }) => {
  ...
  const Images = imageUrls.map((urls, i) => {
    ...
    return (
      <>
        <div {...handlers} >
          <Card raised={true} sx={{ display: 'flex', margin: '0 auto', width: { xs: '100vw', md: '50vw' }, height: '100dvh', backgroundColor: '#333333', zIndex: 0, position: 'absolute', top: 0, left: { xs: 0, md: '25%' } }} >
            <CardMedia component={'div'} sx={{ zIndex: 1, flexShrink: 1, position: 'absolute', top: 0, margin: '0 auto', width: '100%', minHeight:'75%' }} >
            ...
            <CardContent sx={{ color: '#ffffff', width: '95%', minHeight:'25%', zIndex: 0, position: 'relative', top: {xs:'60%', md:'75%'}, left:'0', margin: '0 auto' }} className="MuiDialog-paper" >
